        ...
        interaction.editReply({
            embeds: [
                embed.setTitle(`**${client.user.username} - Stats**`)
                    .setDescription('')
                    .addFields(
                        { name: 'Uptime', value: `${days} day${((days===1)?'':'s')}, ${hours} hour${((hours===1)?'':'s')}, ${minutes} minute${((minutes===1)?'':'s')} & ${seconds} second${((seconds===1)?'':'s')}` },
                        ...
                        { name: 'RAM usage', value: `${Math.round((process.memoryUsage().heapUsed / 1024 / 1024) * 100) / 100}MB\n`, inline: true },
                        { name: 'CPU usage', value: `${process.cpuUsage(usage).user} %`, inline: true})
                    .setColor('DARK_GREEN'),
            ],
        })
    },
